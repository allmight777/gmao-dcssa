...
@section('page-title', 'Modifier le contrat ' . $contrat->Numero_Contrat)

@section('page-actions')
<a href="{{ route('admin.contrats.show', $contrat->ID_Contrat) }}" class="btn-return me-2">
...
<div class="edit-contrat-container">
    ...
    <form action="{{ route('admin.contrats.update', $contrat->ID_Contrat) }}" method="POST" enctype="multipart/form-data" class="contrat-form" id="contratForm">
        @csrf
        @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="text" id="Libelle" name="Libelle"
                               value="{{ old('Libelle', $contrat->Libelle) }}"
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="Type" name="Type" class="form-select" required>
                            ...
                            <option value="preventive" {{ old('Type', $contrat->Type) == 'preventive' ? 'selected' : '' }}>Maintenance préventive</option>
                            <option value="corrective" {{ old('Type', $contrat->Type) == 'corrective' ? 'selected' : '' }}>Maintenance corrective</option>
                            <option value="globale" {{ old('Type', $contrat->Type) == 'globale' ? 'selected' : '' }}>Maintenance globale</option>
                            <option value="garantie" {{ old('Type', $contrat->Type) == 'garantie' ? 'selected' : '' }}>Garantie</option>
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="ID_Fournisseur" name="ID_Fournisseur" class="form-select" required>
                            ...
                            @foreach($fournisseurs as $fournisseur)
                                <option value="{{ $fournisseur->id }}" {{ old('ID_Fournisseur', $contrat->ID_Fournisseur) == $fournisseur->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="Periodicite_Interventions" name="Periodicite_Interventions" class="form-select" required>
                            ...
                            <option value="hebdomadaire" {{ old('Periodicite_Interventions', $contrat->Periodicite_Interventions) == 'hebdomadaire' ? 'selected' : '' }}>Hebdomadaire</option>
                            <option value="mensuelle" {{ old('Periodicite_Interventions', $contrat->Periodicite_Interventions) == 'mensuelle' ? 'selected' : '' }}>Mensuelle</option>
                            <option value="trimestrielle" {{ old('Periodicite_Interventions', $contrat->Periodicite_Interventions) == 'trimestrielle' ? 'selected' : '' }}>Trimestrielle</option>
                            <option value="semestrielle" {{ old('Periodicite_Interventions', $contrat->Periodicite_Interventions) == 'semestrielle' ? 'selected' : '' }}>Semestrielle</option>
                            <option value="annuelle" {{ old('Periodicite_Interventions', $contrat->Periodicite_Interventions) == 'annuelle' ? 'selected' : '' }}>Annuelle</option>
                            <option value="ponctuelle" {{ old('Periodicite_Interventions', $contrat->Periodicite_Interventions) == 'ponctuelle' ? 'selected' : '' }}>Ponctuelle</option>
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="number" id="Delai_Intervention_Garanti" name="Delai_Intervention_Garanti"
                               value="{{ old('Delai_Intervention_Garanti', $contrat->Delai_Intervention_Garanti) }}" required min="1">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="number" step="0.01" id="Montant" name="Montant"
                               value="{{ old('Montant', $contrat->Montant) }}" required>
                    </div>
                    @error('Montant')
                        <span class="error">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="Devise" name="Devise" class="form-select" required>
                            <option value="XOF" {{ old('Devise', $contrat->Devise) == 'XOF' ? 'selected' : '' }}>FCFA (XOF)</option>
                            <option value="EUR" {{ old('Devise', $contrat->Devise) == 'EUR' ? 'selected' : '' }}>Euro (EUR)</option>
                            <option value="USD" {{ old('Devise', $contrat->Devise) == 'USD' ? 'selected' : '' }}>Dollar (USD)</option>
                        </select>
                    </div>
                    @error('Devise')
                        <span class="error">{{ $message }}</span>
                ...
                <div class="form-group checkbox-group">
                    <div class="checkbox-wrapper">
                        <input type="checkbox" id="Couverture_Pieces" name="Couverture_Pieces" value="1"
                               {{ old('Couverture_Pieces', $contrat->Couverture_Pieces) ? 'checked' : '' }}>
                ...
                <div class="form-group checkbox-group">
                    <div class="checkbox-wrapper">
                        <input type="checkbox" id="Couverture_Main_Oeuvre" name="Couverture_Main_Oeuvre" value="1"
                               {{ old('Couverture_Main_Oeuvre', $contrat->Couverture_Main_Oeuvre) ? 'checked' : '' }}>
                ...
                <div class="form-group full-width">
                    ...
                    <div class="input-with-icon">
                        ...
                        <textarea id="Exclusions" name="Exclusions" rows="3"
                                  placeholder="Éléments exclus de la couverture...">{{ old('Exclusions', $contrat->Exclusions) }}</textarea>
                    </div>
                    @error('Exclusions')
                        <span class="error">{{ $message }}</span>
                    @enderror
                        ...
                        <select id="equipements" name="equipements[]" class="form-select" multiple size="5">
                            @foreach($equipements as $equipement)
                                <option value="{{ $equipement->id }}"
                                    {{ in_array($equipement->id, old('equipements', $equipementsSelectionnes)) ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="date" id="Date_Alerte_Renouvellement" name="Date_Alerte_Renouvellement"
                               value="{{ old('Date_Alerte_Renouvellement', $contrat->Date_Alerte_Renouvellement ? $contrat->Date_Alerte_Renouvellement->format('Y-m-d') : '') }}">
                    </div>
                ...
                <div class="form-group full-width">
                    ...
                    <div class="input-with-icon">
                        ...
                        <textarea id="Conditions_Particulieres" name="Conditions_Particulieres" rows="3"
                                  placeholder="Conditions spécifiques du contrat...">{{ old('Conditions_Particulieres', $contrat->Conditions_Particulieres) }}</textarea>
                    </div>
                ...
                <div class="form-group full-width">
                    ...
                    <div class="input-with-icon">
                        ...
                        <textarea id="Notes_Internes" name="Notes_Internes" rows="3"
                                  placeholder="Notes pour usage interne...">{{ old('Notes_Internes', $contrat->Notes_Internes) }}</textarea>
                    </div>
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                @if($contrat->chemin_document)
                    <div class="col-12 mb-3">
                        <div class="current-document">
                            ...
                            Document actuel : <strong>{{ $contrat->fichier_original }}</strong>
                            <a href="{{ Storage::url($contrat->chemin_document) }}" class="btn-view" target="_blank">
                ...
                <div class="form-group full-width">
                    <label for="document">Remplacer le document (PDF, DOC, DOCX)</label>
                    ...
                    <small class="form-hint">Laissez vide pour conserver le document actuel. Taille max: 10 Mo</small>
                    @error('document')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div class="preview-section">
            ...
            <div class="preview-card">
                <div class="preview-header">
                    <div class="preview-title" id="previewTitle">{{ $contrat->Libelle }}</div>
                    <div class="preview-badge" id="previewType">{{ $contrat->Type }}</div>
                </div>
                <div class="preview-body">
                    <div class="preview-row">
                        <span class="preview-label">Fournisseur:</span>
                        <span class="preview-value" id="previewFournisseur">{{ $contrat->fournisseur->raison_sociale ?? '-' }}</span>
                    </div>
                    <div class="preview-row">
                        <span class="preview-label">Période:</span>
                        <span class="preview-value" id="previewPeriode">{{ $contrat->Date_Debut->format('d/m/Y') }} au {{ $contrat->Date_Fin->format('d/m/Y') }}</span>
                    </div>
                    <div class="preview-row">
                        <span class="preview-label">Montant:</span>
                        <span class="preview-value" id="previewMontant">{{ number_format($contrat->Montant, 0, ',', ' ') }} {{ $contrat->Devise }}</span>
                    </div>
                    <div class="preview-row">
                        <span class="preview-label">Périodicité:</span>
                        <span class="preview-value" id="previewPeriodicite">{{ $contrat->Periodicite_Interventions }}</span>
                    </div>
                    <div class="preview-row">
                        <span class="preview-label">Couverture:</span>
                        <span class="preview-value" id="previewCouverture">
                            @if($contrat->Couverture_Pieces && $contrat->Couverture_Main_Oeuvre)
                                Pièces + Main d'œuvre
                            @elseif($contrat->Couverture_Pieces)
                                Pièces uniquement
                            @elseif($contrat->Couverture_Main_Oeuvre)
        ...
        <div class="form-actions">
            ...
            <div class="action-buttons">
                <a href="{{ route('admin.contrats.show', $contrat->ID_Contrat) }}" class="btn-cancel">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                text: 'Êtes-vous sûr de vouloir modifier ce contrat de maintenance ?',
                icon: 'question',
        ...
        const alerteInput = document.getElementById('Date_Alerte_Renouvellement');
        if (alerteInput) {
            alerteInput.addEventListener('change', function() {
