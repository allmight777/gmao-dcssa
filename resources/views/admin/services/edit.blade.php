...
<div class="create-service-container">
    ...
    <form method="POST" action="{{ route('admin.services.update', $service) }}" class="service-form" id="serviceForm">
        ...
        <div class="form-section">
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="type" name="type" class="form-select" required onchange="toggleCustomType()">
                            ...
                            @foreach($types as $key => $label)
                                <option value="{{ $key }}" 
                                    @if($isCustomType && $key == 'autre')
                                        selected
                                    @elseif(!$isCustomType && old('type', $service->type) == $key)
                                        selected
                                    @endif>
                                    {{ $label }}
                                </option>
            ...
            <div class="form-row">
                <div class="form-group" id="typeCustomGroup" style="{{ $isCustomType ? 'display: block;' : 'display: none;' }}">
                    <label for="type_custom">Spécifiez le type *</label>
                    <div class="input-with-icon">
                        <i class="fas fa-edit"></i>
                        <input type="text" id="type_custom" name="type_custom" 
                               value="{{ old('type_custom', $isCustomType ? $service->original_type : '') }}" 
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleCustomType() {
            ...
            const customTypeInput = document.getElementById('type_custom');
            
            if (typeSelect.value === 'autre') {
                customTypeGroup.style.display = 'block';
                customTypeInput.required = true;
                
                // Afficher une alerte d'information si le champ est vide
        ...
        // Auto-génération du code géographique si vide
        const typeSelect = document.getElementById('type');
        ...
        form.addEventListener('submit', function(e) {
            ...
            const originalData = {
                type: @if($isCustomType)'autre'@else'{{ $service->type }}'@endif,
                type_custom: @if($isCustomType)'{{ $service->original_type }}'@else''@endif,
                nom: '{{ $service->nom }}',
                parent_id: '{{ $service->parent_id }}',
                code_geographique: '{{ $service->code_geographique }}',
            ...
            for (let [key, value] of formData.entries()) {
                ...
                if (key === 'type_custom' && !value && originalData.type !== 'autre') {
                    continue;
                }
            ...
            let message = 'Êtes-vous sûr de vouloir mettre à jour ce service/localisation ?';
            
            // Ajouter des informations supplémentaires
            const parentSelect = document.getElementById('parent_id');
            const responsableSelect = document.getElementById('responsable_id');
            
            if (parentSelect.value !== originalData.parent_id || responsableSelect.value !== originalData.responsable_id) {
                message += '\n\n';
                if (parentSelect.value !== originalData.parent_id) {
                    const parentText = parentSelect.options[parentSelect.selectedIndex]?.text || 'Aucun parent';
                    const oldParentText = originalData.parent_id ? 
                        Array.from(parentSelect.options).find(opt => opt.value === originalData.parent_id)?.text || 'Ancien parent' : 
                        'Aucun parent';
                    message += `• Parent : ${oldParentText} → ${parentText}\n`;
                }
                if (responsableSelect.value !== originalData.responsable_id) {
                    const responsableText = responsableSelect.options[responsableSelect.selectedIndex]?.text || 'Aucun responsable';
                    const oldResponsableText = originalData.responsable_id ? 
                        Array.from(responsableSelect.options).find(opt => opt.value === originalData.responsable_id)?.text || 'Ancien responsable' : 
                        'Aucun responsable';
                    message += `• Responsable : ${oldResponsableText} → ${responsableText}\n`;
                }
            }
