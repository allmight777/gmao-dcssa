...
<div class="page-actions">
    <style>
        ...
        /* Bouton bleu pour "Nouvel équipement" */
        .btn-blue {
    ...
    <!-- Bouton bleu -->
    <a href="{{ route('inventaire.equipements.create') }}" class="btn-action btn-blue">
        <i class="fas fa-plus"></i> Nouvel équipement
    </a>

    <!-- Bouton vert -->
    <a href="#" class="btn-action btn-green"
       onclick="event.preventDefault(); document.getElementById('rapportForm').submit();">
                ...
                <div class="filter-grid">
                        ...
                        <select id="type_equipement_id" name="type_equipement_id">
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type->id }}"
                                    {{ request('type_equipement_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->libelle }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="filter-group">
                        ...
                        <select id="etat" name="etat">
                            ...
                            <option value="hors_service" {{ request('etat') == 'hors_service' ? 'selected' : '' }}>Hors
                                service</option>
                        </select>
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <select id="localisation_id" name="localisation_id">
                            ...
                            @foreach ($localisations as $loc)
                                <option value="{{ $loc->id }}"
                                    {{ request('localisation_id') == $loc->id ? 'selected' : '' }}>
                                    {{ $loc->nom }}
                                </option>
                            @endforeach
                        </select>
                                ...
                                <td>
                                    <div class="date-info">
                                        ...
                                        @if ($equipement->date_mise_service)
                                            <small>Mise service:
                                                {{ $equipement->date_mise_service->format('d/m/Y') }}</small>
                                        @endif
                                    </div>
                                </td>
                                <td>
            ...
            <form id="reformeForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="motif_reforme" name="motif_reforme" rows="3" placeholder="Décrivez le motif de la réforme..."
                            required></textarea>
                    </div>

                    <div class="form-group">
                        <label for="date_reforme">Date de réforme *</label>
                        <input type="date" id="date_reforme" name="date_reforme" value="{{ date('Y-m-d') }}"
                            required>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            Object.keys(stats.par_type).forEach((label, index) => {
                ...
                span.innerHTML =
                    `<span style="background-color: ${typeData.datasets[0].backgroundColor[index]};"></span> ${label}`;
