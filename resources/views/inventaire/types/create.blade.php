...
@section('title', 'Créer un nouveau type d\'équipement')

@section('page-title', 'Créer un nouveau type d\'équipement')
...
<div class="create-type-container">
    ...
    <form method="POST" action="{{ route('inventaire.types.store') }}" class="type-form" id="typeForm">
        @csrf
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="classe" name="classe" class="form-select">
                            ...
                            @foreach($classes as $key => $label)
                                <option value="{{ $key }}" {{ old('classe') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-with-icon">
                        ...
                        <select id="risque" name="risque" class="form-select" required>
                            @foreach($risques as $key => $label)
                                <option value="{{ $key }}" {{ old('risque') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="number" id="duree_vie_standard" name="duree_vie_standard" 
                               value="{{ old('duree_vie_standard') }}" 
                               placeholder="Ex: 60"
                               min="0"
                               step="1">
                    </div>
                    <small class="form-hint">Durée de vie théorique en mois (ex: 60 mois = 5 ans)</small>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="number" id="periodicite_maintenance" name="periodicite_maintenance" 
                               value="{{ old('periodicite_maintenance') }}" 
                               placeholder="Ex: 12"
                               min="0"
                               step="1">
                    </div>
                    <small class="form-hint">Fréquence recommandée des maintenances préventives</small>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <textarea id="description" name="description" rows="5" 
                                  placeholder="Description complète du type d'équipement, spécifications techniques, particularités..."
                                  class="form-control">{{ old('description') }}</textarea>
                    </div>
                    <small class="form-hint">Informations complémentaires sur ce type d'équipement</small>
                    @error('description')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div class="risk-indicator">
            ...
            <div class="risk-preview">
                <div class="risk-level" id="riskPreview">
                    <div class="risk-info">
                        <div class="risk-label" id="riskLabel">Faible</div>
...
<style>
    ...
    .classification-section { animation-delay: 0.2s; }
    .maintenance-section { animation-delay: 0.3s; }
    .description-section { animation-delay: 0.4s; }
    .risk-indicator { animation-delay: 0.5s; }
</style>
@endpush
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Générer un code type automatique
        function generateCodeType() {
            const codeInput = document.getElementById('code_type');
        ...
        // Générer le code au focus si vide
        document.getElementById('code_type').addEventListener('focus', function() {
            if (!this.value.trim()) {
                generateCodeType();
            }
        });
        ...
        form.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                text: 'Êtes-vous sûr de vouloir créer ce type d\'équipement ?',
                icon: 'question',
        ...
        // Calcul automatique de la maintenance si durée de vie renseignée
        const dureeVieInput = document.getElementById('duree_vie_standard');
        const maintenanceInput = document.getElementById('periodicite_maintenance');
        
        dureeVieInput.addEventListener('blur', function() {
            if (this.value && !maintenanceInput.value) {
                // Suggérer une périodicité basée sur la durée de vie
                const dureeVie = parseInt(this.value);
                if (dureeVie >= 120) { // 10 ans ou plus
                    maintenanceInput.value = 24; // 2 ans
                } else if (dureeVie >= 60) { // 5 ans ou plus
                    maintenanceInput.value = 12; // 1 an
                } else if (dureeVie >= 36) { // 3 ans ou plus
                    maintenanceInput.value = 6; // 6 mois
                } else {
                    maintenanceInput.value = 3; // 3 mois par défaut
                }
            }
        });
    });
